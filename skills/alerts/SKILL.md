...
Ao enviar alertas para webhooks, é possível que ocorram erros de rede. Nesse caso, o `trackio.alert()` deve tentar novamente após um tempo de espera.
```python
import time

def send_alert_to_webhook(alert):
    try:
...
Ao armazenar alertas no banco de dados, é possível que ocorram erros de conexão. Nesse caso, o `trackio.alert()` deve tentar novamente após um tempo de espera.
```python
import time
...
*   **Alertas duplicados**: Se um alerta for enviado múltiplas vezes com o mesmo título e texto, o `trackio.alert()` deve apenas atualizar o alerta existente em vez de criar um novo.
*   **Alertas com campos vazios**: Se um alerta for enviado com campos vazios (como título ou texto), o `trackio.alert()` deve lançar uma exceção para evitar armazenar alertas inválidos.
*   **Alertas com níveis de prioridade inválidos**: Se um alerta for enviado com um nível de prioridade inválido, o `trackio.alert()` deve lançar uma exceção para evitar armazenar alertas inválidos.
```python
def validate_alert(alert):
    if not alert.title or not alert.text:
        raise ValueError("Título e texto do alerta são obrigatórios")
    if alert.level not in [trackio.AlertLevel.INFO, trackio.AlertLevel.WARN, trackio.AlertLevel.ERROR]:
        raise ValueError("Nível de prioridade do alerta é inválido")
