...
MCP equivalents (signatures may vary by server version — check the actual tool listing): `hf_jobs("ps")`, `hf_jobs("logs", {"job_id": ...})`, `hf_jobs("cancel", {"job_id": ...})`.
...
*   **Token de Autenticação Inválido**: Verifique se o token de autenticação está correto e se tem permissões de escrita.
*   **Token de Autenticação Expirado**: Verifique se o token de autenticação está expirado e renove-o se necessário.

### Exceções de Rede

*   **Conexão de Rede Instável**: Verifique se a conexão de rede está estável e se o servidor está acessível.
*   **Erro de Conexão de Rede**: Verifique se o erro de conexão de rede é temporário e tente novamente.

### Exceções de Jobs

*   **Job Cancelado**: Verifique se o job foi cancelado e se os recursos foram liberados.
*   **Job em Execução**: Verifique se o job está em execução e se os recursos estão sendo utilizados.

### Exceções de Dados

*   **Dados Inválidos**: Verifique se os dados estão válidos e se estão no formato correto.
*   **Dados Faltantes**: Verifique se os dados estão completos e se não há dados faltantes.

### Exceções de Sistema

*   **Sistema Operacional Inválido**: Verifique se o sistema operacional está correto e se está configurado corretamente.
*   **Recursos Insuficientes**: Verifique se os recursos estão suficientes para executar o job e se os recursos estão sendo utilizados de forma eficiente.

Ao tratar essas exceções e edge cases, é importante ter um plano de contingência e um sistema de logging robusto para identificar e resolver problemas de forma eficiente. Além disso, é fundamental ter uma documentação clara e atualizada para ajudar a resolver problemas e melhorar a experiência do usuário.