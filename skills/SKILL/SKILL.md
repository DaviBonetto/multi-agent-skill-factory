...
### Exceção de Permissão Negada

Se o usuário não tiver permissão para criar um papel, o script deve interromper a execução e exibir uma mensagem de erro clara.

```bash
if ! python3 scripts/create_role.py "<role-name>" "<model-bucket>"; then
  echo "Erro: não foi possível criar o papel. Verifique as permissões do usuário."
  exit 1
fi
```

### Exceção de Papel Não Encontrado

Se o script não encontrar um papel existente, ele deve criar um novo papel com as permissões mínimas necessárias.

```bash
if ! python3 scripts/check_role.py; then
  echo "Papel não encontrado. Criando um novo papel..."
  python3 scripts/create_role.py "<role-name>" "<model-bucket>"
fi
```

### Exceção de Conexão de Rede

Se o script não conseguir se conectar à AWS, ele deve exibir uma mensagem de erro clara e interromper a execução.

```bash
if ! aws sts get-caller-identity; then
  echo "Erro: não foi possível se conectar à AWS. Verifique as configurações de rede e credenciais."
  exit 1
fi
```

Essas exceções e edge cases devem ser tratados para garantir que o script seja robusto e forneça mensagens de erro claras e úteis.