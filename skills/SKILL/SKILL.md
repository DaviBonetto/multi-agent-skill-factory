...
* Use `hf_whoami` para verificar a autenticação.

### Erros de Conexão

* Verifique se a conexão com o servidor MCP está estabelecida.
* Tente novamente após alguns minutos se a conexão falhar.

### Erros de Entrada Inválida

* Verifique se os parâmetros de entrada estão corretos e no formato esperado.
* Use `try-except` para capturar e tratar erros de entrada inválida.

### Erros de Limitação de Recursos

* Verifique se os recursos (como memória ou CPU) estão disponíveis para executar a tarefa.
* Use `hf_jobs` com parâmetros de recursos adequados para evitar limitações.

### Edge Cases

* Trate casos de entrada vazia ou nula.
* Trate casos de saída inesperada ou erro.
* Use `try-except` para capturar e tratar exceções inesperadas.
