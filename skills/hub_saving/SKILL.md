...
*   Verifique se o token `HF_TOKEN` está correto e válido.
*   Certifique-se de que o token tem permissões de escrita para o repositório.
*   Use `try-except` para capturar erros de autenticação e fornecer feedback ao usuário.
...
*   Use `try-except` para capturar erros de conexão e fornecer feedback ao usuário.
*   Tente novamente a operação após um tempo.
...
while True:
    try:
        ...
        print(f"Erro ao fazer upload: {e}. Tentando novamente em 5 segundos.")
        time.sleep(5)
```
...
*   Use `try-except` para capturar erros de repositório e fornecer feedback ao usuário.
*   Crie o repositório se ele não existir.
...
try:
    ...
    print(f"Erro ao fazer upload: {e}. Criando repositório...")
    api.create_repo(
        repo_id="username/results",
        repo_type="dataset",
        private=False,
