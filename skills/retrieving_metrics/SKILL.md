...
O tratamento de exceções e edge cases é fundamental para garantir a robustez e confiabilidade do sistema. Aqui estão alguns exemplos de como lidar com exceções e edge cases:

* **Exceção de projeto não encontrado**: Se o projeto não for encontrado, o sistema deve retornar um erro claro e conciso, com um código de status apropriado.
* **Exceção de execução não encontrada**: Se a execução não for encontrada, o sistema deve retornar um erro claro e conciso, com um código de status apropriado.
* **Exceção de métrica não encontrada**: Se a métrica não for encontrada, o sistema deve retornar um erro claro e conciso, com um código de status apropriado.
* **Edge case de projeto vazio**: Se o projeto estiver vazio, o sistema deve retornar um erro claro e conciso, com um código de status apropriado.
* **Edge case de execução vazia**: Se a execução estiver vazia, o sistema deve retornar um erro claro e conciso, com um código de status apropriado.

Além disso, o sistema deve ser capaz de lidar com situações de erro inesperadas, como falhas de rede ou problemas de banco de dados. Nesses casos, o sistema deve retornar um erro claro e conciso, com um código de status apropriado, e registrar o erro para que possa ser investigado e resolvido.

Exemplos de como lidar com exceções e edge cases em código:
```bash
try:
    # Tente executar a ação
    project = trackio.get_project(project_name)
except trackio.ProjectNotFoundError:
    # Se o projeto não for encontrado, retorne um erro claro e conciso
...
try:
    # Tente executar a ação
    run = trackio.get_run(project_name, run_name)
except trackio.RunNotFoundError:
    # Se a execução não for encontrada, retorne um erro claro e conciso
    print(f"Erro: Execução '{run_name}' não encontrada no projeto '{project_name}'.")
    sys.exit(1)
```
...
try:
    # Tente executar a ação
    metric = trackio.get_metric(project_name, run_name, metric_name)
except trackio.MetricNotFoundError:
    # Se a métrica não for encontrada, retorne um erro claro e conciso
    print(f"Erro: Métrica '{metric_name}' não encontrada na execução '{run_name}' do projeto '{project_name}'.")
    sys.exit(1)
```
