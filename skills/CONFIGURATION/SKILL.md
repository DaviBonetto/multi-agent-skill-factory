...
8. [⚠️ Tratamento de Exceções e Edge Cases](#tratamento-de-exceções-e-edge-cases)
...
try {
  ...
  if (error instanceof Error && error.message.includes('cache')) {
    console.error('Erro de cache. Limpe o cache e tente novamente.');
  } else {
    throw error;
  }
}
...
try {
  ...
  if (error instanceof Error && error.message.includes('model')) {
    console.error('Erro de modelo. Verifique se o modelo está correto e carregado.');
  } else {
    throw error;
  }
}
