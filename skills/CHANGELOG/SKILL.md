...
- **Tratamento de Erros em `server.cjs`**: Em caso de falha na renomeação de `server.js` para `server.cjs`, o servidor deve registrar um erro e fornecer instruções para solução do problema.
- **Monitoramento de PID em Ambientes de Desenvolvimento**: Em ambientes de desenvolvimento, o monitoramento de PID deve ser configurável para evitar interferências com outros processos em execução.
- **Timeout de Idle**: O timeout de idle de 30 minutos deve ser configurável para atender a diferentes necessidades de desenvolvimento e produção.
- **Falha no Escalamento de `SIGKILL`**: Em caso de falha no escalamento para `SIGKILL`, o servidor deve registrar um erro e fornecer instruções para solução do problema.
- **Compatibilidade com Versões Anteriores**: A restauração da escolha entre subagent-driven-development e executing-plans deve ser compatível com versões anteriores do sistema, garantindo que os usuários possam migrar sem perda de funcionalidade.