...
- **Conexão perdida:** ao perder a conexão com um cliente, o servidor remove o cliente da lista de broadcast e continua operando normalmente.
- **Erros de handshake:** se ocorrer um erro durante o handshake, o servidor fecha a conexão com o cliente.

### Exceções de Arquivos

- **Arquivos não encontrados:** se um arquivo não for encontrado no diretório de tela, o servidor retorna um erro 404.
- **Erros de leitura:** se ocorrer um erro ao ler um arquivo, o servidor loga o erro e continua operando normalmente.

### Exceções de JSON

- **JSON malformado:** se um cliente enviar um JSON malformado, o servidor loga o erro e continua operando normalmente.
- **Propriedades desconhecidas:** se um cliente enviar um JSON com propriedades desconhecidas, o servidor ignora essas propriedades e continua operando normalmente.

### Outros Edge Cases

- **Múltiplos clientes:** o servidor suporta múltiplos clientes conectados simultaneamente.
- **Conexões simultâneas:** o servidor suporta conexões simultâneas de diferentes clientes.
- **Reconexão:** se um cliente se desconectar e reconectar, o servidor reconhece o cliente e continua operando normalmente.