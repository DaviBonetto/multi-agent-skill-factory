...
Implemente tratamento de exceções e edge cases para garantir a robustez do programa.

**Do:**
- Adicione tratamento de exceções para erros de entrada/saída, como arquivos não encontrados ou permissões insuficientes
- Implemente validação de entrada para evitar erros de formato ou valores inválidos
- Adicione testes para cobrir casos de erro e edge cases

**Exemplos de Edge Cases:**
- Tamanho de entrada muito grande ou muito pequeno
- Profundidade de recursão muito grande ou muito pequena
- Caracteres inválidos ou não suportados
- Erros de inicialização ou configuração

**Verificar:**
- O programa lida corretamente com exceções e edge cases
- Os testes cobrem todos os casos de erro e edge cases
- O programa fornece mensagens de erro claras e úteis para o usuário
...
func Generate(size, depth int, char rune) ([]string, error) {
    ...
    if depth < 0 {
        return nil, errors.New("profundidade deve ser não negativa")
    }
    // ...
}
```
**Verificar:**
- O código lida corretamente com exceções e edge cases
- Os testes cobrem todos os casos de erro e edge cases
- O programa fornece mensagens de erro claras e úteis para o usuário