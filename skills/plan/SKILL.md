...
**Do:**
- Adicionar tratamento de exceções para erros de entrada/saída
- Adicionar verificações para edge cases, como:
  - Tamanho de entrada muito grande
  - Profundidade de entrada muito grande
  - Número de iterações muito grande
- Adicionar testes para esses casos

**Verify:**
- O programa lida corretamente com exceções e edge cases
- Testes para esses casos passam

Exemplos de edge cases e tratamento de exceções:

*   Tratamento de exceções para erros de entrada/saída:
    *   Utilizar `try-catch` para capturar erros de leitura/escrita de arquivos
    *   Utilizar `try-catch` para capturar erros de rede
*   Verificações para edge cases:
    *   Verificar se o tamanho de entrada é muito grande e retornar um erro se necessário
    *   Verificar se a profundidade de entrada é muito grande e retornar um erro se necessário
    *   Verificar se o número de iterações é muito grande e retornar um erro se necessário
...
```
Exemplo de código para verificações de edge cases:
```go
func sierpinski(size, depth int) {
    if size < 1 {
        fmt.Println("Erro: tamanho de entrada deve ser maior que 0")
        return
    }
    if depth < 0 {
        fmt.Println("Erro: profundidade de entrada deve ser maior ou igual a 0")
        return
    }
    // ...
}