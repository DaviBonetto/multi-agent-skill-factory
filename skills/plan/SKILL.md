...
**Do:**
- Adicionar tratamento de erros para casos como:
  - Entradas inválidas (ex: tamanho negativo)
  - Falhas de inicialização do Cobra
  - Erros de leitura/escrita de arquivos
- Implementar testes para esses casos
- Verificar que o programa lida corretamente com esses casos

**Verify:**
- O programa lida corretamente com exceções e edge cases
- Testes para esses casos passam com sucesso

Exemplos de edge cases a considerar:
- Tamanho de entrada muito grande
- Profundidade de recursão muito grande
- Caracteres inválidos para o `--char` flag
- Comandos inválidos ou desconhecidos

Exemplos de tratamento de exceções:
- Usar `errors.New` para criar erros personalizados
- Usar `log.Fatal` para registrar erros fatais
- Usar `defer` para garantir que recursos sejam liberados em caso de erro

Lembre-se de que a documentação e os testes são fundamentais para garantir a qualidade e a robustez do programa.